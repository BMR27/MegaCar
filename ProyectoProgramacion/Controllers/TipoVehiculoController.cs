...
namespace ProyectoProgramacion.Controllers
{
    public class TipoVehiculoController : Controller
    {
        ...
        public ActionResult EliminarTipoVehiculo(SP_RETORNA_TIPO_VEHICULO_Result ModeloVista)
        {
            string mensaje = string.Empty;
            int filas = 0;
            try
            {
                filas = this.ModeloDB.SP_ELIMINAR_TIPO_VEHICULO(ModeloVista.C_ID_TIPO_VEHICULO);
            }
            catch (Exception error)
            {
            ...
            finally
            {
                if (filas > 0)
                {
                    mensaje = "Exito al eliminar el tipo de vehiculo";
                }else
                {
                    mensaje = "No se pudo eliminar el tipo de vehiculo";
                }
                Response.Write("<script language=javascript>alert('" + mensaje + "');</script>");
            }
            this.ViewBag.Titulo = mensaje;
            return View();
        }
